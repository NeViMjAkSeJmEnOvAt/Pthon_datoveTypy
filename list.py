...
for x in range(1, 2001):
    if x % 200 == 0:
        hundreds.append(x)
print(f'a) hundreds pred upravou: {hundreds}')
#2
ascii = []
x=0
for x in range(1, 50):
...
#vypis
print(f'b) ascii: {ascii}')
print(f'c) hundreds po uprave: {hundreds}')
print(f'd) unique: {unique}')
print(f'e) combine: {combine}')




# ??? 3. cvičení ???
#H a) Přidejte do listu persons ještě n-tice (tuples) dalších 2 žen a 2 mužů.
#H a) Použijte seznam (list) persons a do proměnné women z něj pomocí lambda výrazu i comprehensions vyhledejte všechny ženy.
#H Seznam jmen žen poté vypište na samostatné řádky. Každý vypsaný řádek bude podtržen pomlčkami přesně podle délky jména.
#H b) Použijte seznam (list) persons a do proměnné ipeople z něj pomocí lambda výrazu i comprehensions vyhledejte všechny osoby
#H obsahující ve jméně písmeno "i". Obsah listu ipeople poté převeďte do podoby řetězce, který bude odpovídat struktuře csv souboru.
...
print(f'\n*************************************\nCvičení 3\n*************************************')
persons.append(('Michal', 17, "muž"))
persons.append(('Donald', 49, "muž"))
persons.append(('Pavla', 26, 'žena'))
persons.append(('Nikola', 98, 'žena'))
print(persons)
...
z=0
for x in ipeople:
    print(str(z) + '.', ';', ipeople[z][0], ';', ipeople[z][1], ';',ipeople[z][2])
    z += 1
