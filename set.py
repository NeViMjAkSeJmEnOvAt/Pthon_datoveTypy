...
# Jakmile je sada vytvořena, nemůžete změnit její položky, ale můžete přidávat nové položky.
# Chcete li přidat položku do sady, použijte metodu add().
set_chars.add('V')

# Pokud chcete do sady přidat více než jednu položku, použijte metodu update().
set_chars.update('X', 'Y', 'Z')

# Pokud chcete odebrat položku v sadě, použijte metodu remove() nebo discard().
set_chars.remove('H')
print(f'Proměnná set_chars: {set_chars}')

# Metoda clear() vyčistí sadu
set_chars.clear()
